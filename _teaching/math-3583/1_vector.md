...
<!-- In order for a set to be a vector space, we need to have defined **addition** (i.e., have defined how we add two vectors together) and **scalar multiplication** (i.e. we have defined how we multiply a vector by a scalar).  -->
...
<ol start="3">
  ...
  <li>For every element \(\mathbf{u}\) of \(V\), there exists an element called a <strong>negative</strong> of \(\mathbf{u}\), denoted \(-\mathbf{u}\), such that \(\mathbf{u} + (-\mathbf{u}) = 0\).</li>
</ol>
